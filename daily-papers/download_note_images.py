...
CURL_TIMEOUT = 10
CONCURRENCY = 5
FIGURE_1_RE = re.compile(r"\b(?:fig(?:ure)?\.?\s*0*1|fig(?:ure)?[-_ ]?0*1|fig1|figure1)\b", re.IGNORECASE)


def detect_publisher_site(url: str = "", journal: str = "", title: str = "", doi: str = "") -> str:
    hay = " ".join([url, journal, title, doi]).lower()
    if any(token in hay for token in ["pnas.org", "proc. natl. acad. sci.", "proceedings of the national academy of sciences"]):
        return "pnas"
    if any(token in hay for token in ["elsevier", "sciencedirect.com", "doi.org/10.1016/"]):
        return "elsevier"
    if any(token in hay for token in ["academic.oup.com", "oup.com", "oxford academic"]):
        return "oup"
    return ""
...
def extract_note_context(text: str) -> dict:
    fm = parse_frontmatter(text)
    return {
        "local_pdf": fm.get("local_pdf", ""),
        "publisher_site": fm.get("publisher_site", ""),
...
async def try_pdf_extract_from_path(pdf_path: Path, assets_dir: Path, method_name: str,
...
async def try_pdf_extract(arxiv_id: str, assets_dir: Path, method_name: str,
    ...
    try:
        ...
        if Path(pdf_path).exists():
            return await try_pdf_extract_from_path(Path(pdf_path), assets_dir, method_name, fig_num, sem)
    except (asyncio.TimeoutError, Exception):
        pass
    return None
...
async def process_note(note_path: Path) -> dict:
    ...
    note_context = extract_note_context(text)
    ...
    for i, (img, is_ok) in enumerate(zip(images, reachable)):
        ...
        if not ok:
            local_pdf_raw = (note_context.get("local_pdf", "") or "").strip()
            local_pdf = Path(local_pdf_raw).expanduser() if local_pdf_raw else None
            if local_pdf is not None and not local_pdf.is_absolute():
                local_pdf = note_path.parent / local_pdf
            site_hint = detect_publisher_site(
                note_context.get("web_url", ""),
                note_context.get("journal", ""),
                note_context.get("title", ""),
                note_context.get("doi_url", ""),
            ) or (note_context.get("publisher_site", "") or "").strip()
            if local_pdf is not None and local_pdf.exists():
                print(f"  [PDF fallback] local:{local_pdf.name} fig{fig_num}")
                pdf_path = await try_pdf_extract_from_path(local_pdf, assets_dir, method_name, fig_num, sem)
                if pdf_path:
                    pdf_path.rename(local_path)
                    ok = True
            if not ok and site_hint == "pnas":
                arxiv_id = extract_arxiv_id(img["url"])
                if arxiv_id:
                    print(f"  [PDF fallback] pnas-heuristic arxiv:{arxiv_id} fig{fig_num}")
