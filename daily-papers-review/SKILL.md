...
```powershell
python ..\daily-papers\build_review.py "{TEMP_DIR}\daily_papers_enriched.json" --date YYYY-MM-DD
```
...
不要改动这些结构化字段：`分级`、`得分`、`来源`、`日期`、`期刊/平台`、`ID`、`链接`、`关键词`、论文排序和标题。除非用户明确要求人工重排。

交付前必须检查 final：

- 不得残留 `TODO_AGENT_REWRITE`
- 不得残留 `结构化草稿`、`本轮主列表共`、`当前自动分桶`、`命中主题词`、`当前筛选得分`
- 正文主体必须是中文；基因名、物种名、数据库、方法名和必要术语可以保留英文
- 摘要未提供的信息只能写 `摘要未说明` 或 `需要看全文确认`，不能补编样本量、实验设计或机制结论
...
`推荐锐评` 是对当天整批候选的整体判断，不是栏目导语，不是流程说明，也不是“我接下来要给你看什么”；不要另建 `今日锐评` 标题。
...
- `作者真正想回答的是……`
- `这篇真正有意思的不是……而是……`
- 基因名、物种拉丁名、数据库名、方法名等必要英文术语；但句子主体必须是中文
