...
1. 把 final 内容直接写入（且只写入）：

`{DAILY_PAPERS_PATH}/YYYY-MM-DD-论文推荐.md`

**命名铁律**：正式推荐文件名只能是 `YYYY-MM-DD-论文推荐.md`，绝不要创建 `.final.md`、`.reviewed.md` 或任何其他中间文件名。如果发现 vault 下存在 `YYYY-MM-DD-论文推荐.final.md`，立即删除它。
...
- **文件命名**：正式推荐文件只能叫 `YYYY-MM-DD-论文推荐.md`，任何 `.final.md` / `.reviewed.md` 等命名均不允许出现在 vault 目录下
